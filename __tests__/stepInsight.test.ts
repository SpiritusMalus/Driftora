...
describe('stepInsight', () => {
  it('never promotes the 10,000-steps myth (both locales)', () => {
    for (const steps of [0, 3000, 7000, 9000, 15000]) {
      for (const lng of ['ru', 'en'] as const) {
        const s = String(i18n.t(stepInsight(steps, 7000), { lng }));
  ...
  it('acknowledges hitting the personal goal', () => {
    expect(stepInsight(7200, 7000)).toBe('insights.steps.goalMet');
    expect(String(i18n.t(stepInsight(7200, 7000)))).toContain('цель');
  });

  it('is gentle, not shaming, at low counts', () => {
    expect(String(i18n.t(stepInsight(800, 7000)))).not.toContain('провал');
  });
});
