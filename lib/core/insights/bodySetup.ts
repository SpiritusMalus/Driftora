import { validBodyFatPct, type GoalMode } from './bodyMetrics';
...
/// pace, so those steps vanish for it instead of showing up disabled.
export function setupSteps(goal: GoalMode): SetupStep[] {
  const base: SetupStep[] = ['birthYear', 'sex', 'height', 'weight', 'bodyFat', 'goal'];
