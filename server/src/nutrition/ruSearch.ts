...
/**
 * Окончания РУССКИХ ПРИЛАГАТЕЛЬНЫХ (полная форма). Существительные-еда таких
 * окончаний не имеют, поэтому набор надёжно делит «гречневая лапша» на
 * определение и главное слово.
 *
 * Двусмысленности сознательно оставлены за бортом: «-ой» бывает и у
 * существительного в творительном («с лапшой»), а «-ая»/«-ое»/«-ые» — нет.
 * Ошибка в сторону «это существительное» безопаснее: правило ниже только
 * ПОНИЖАЕТ находку, и лишняя осторожность стоит нам пропущенного понижения,
 * а не выброшенной верной строки.
 */
const ADJECTIVE_ENDINGS = ['ая', 'яя', 'ое', 'ее', 'ые', 'ие', 'ый', 'ий', 'ую', 'юю', 'ого', 'его', 'ыми', 'ими'];

/**
 * Похоже ли слово на русское прилагательное в полной форме.
 *
 * Нужно, чтобы отличить ПОТЕРЮ ГЛАВНОГО СЛОВА от нормального отбрасывания
 * уточнения. «борщ украинский» → «борщ» — определение отпало, еда та же.
 * «гречневая лапша» → «гречка» — совпало ОПРЕДЕЛЕНИЕ, а главное слово (лапша)
 * потерялось, и это уже другая еда. По покрытию оба случая одинаковы (1 из 2),
 * различает их только часть речи.
 */
export function looksAdjectiveRu(word: string): boolean {
  const w = word.trim().toLowerCase();
  if (w.length < 5) return false; // «щи», «уха» — короткие слова не прилагательные
  return ADJECTIVE_ENDINGS.some((e) => w.endsWith(e) && w.length - e.length >= 3);
}
