import type { SkurikhinEntry } from './skurikhinTypes.js';

/**
 * Hand-curated RU composition rows for common foods ABSENT from the
 * USDA-SR-Legacy import in `skurikhinData.ts` (that file is auto-generated — do
 * not hand-edit it; add to this list instead). These are national dishes and
 * everyday RU items (супы / салаты / вторые / каши / выпечка) the resolver
 * otherwise missed → fell back to the coarse `estimate`, which the UI shows as
 * "not in our database".
 *
 * Per-100g values are standard published Russian composition figures for the
 * classic recipe of each dish (editorial calorie-table rows, not user recipes);
 * provenance is attributed as 'skurikhin' (curated RU table) so the UI labels
 * them honestly. Composite dishes vary by recipe — these are the reference
 * figures, and the client's "не то?" picker + food_choices memory let the user
 * correct any specific case. Minerals are left empty for now (macros are the
 * figures we can stand behind).
 *
 * FIBRE (пищевые волокна, g/100 g) comes from the same Skurikhin-based RU
 * composition tables, each dish matched to the recipe row whose kcal/БЖУ line up
 * with the figures here. For legume and whole-grain dishes (гороховый/фасолевый
 * суп, гречневая и манная каши) the published dish rows still carry the old
 * Soviet «клетчатка» (crude/Weender fibre, which undercounts true fibre several-
 * fold — see docs/nutrition-science.md §5), so those were recomputed from the raw
 * ingredient's modern AOAC figure. An explicit `fiber: 0` (бульон, лимонады) means
 * "measured, none here" and deliberately blocks the USDA proxy back-fill.
 *
 * LEAST CERTAIN (recipe variants genuinely disagree — worth revisiting before
 * anyone leans on them): суп харчо, голубцы, щавелевый суп, компот из сухофруктов,
 * селёдка под шубой, салат мимоза, крабовый салат, гуляш, пирожок. And котлета:
 * 0.1 assumes the filler-free patty implied by its carb 0 — a standard
 * bread-filler recipe would be ~1.0–1.5 instead.
 *
 * `prepared: true` marks rows that are FINISHED dishes as served (супы, салаты,
 * готовые вторые, каши, выпечка): their per-100g already describes the cooked
 * dish, so the client hides the cooking-method chips («сырое» on a soup reads
 * absurd, and a fry-adjustment would double-count). Products still cooked at
 * home stay unmarked so the chips remain available.
 */
export const CURATED_RU: SkurikhinEntry[] = [
  // — Супы (классические рецептуры; на 100 г готового блюда) —
  { name: 'борщ', aliases: ['борщ', 'борщ украинский'], source: 'skurikhin', prepared: true,
    per100: { kcal: 49, prot: 1.1, fat: 2.2, carb: 6.7, fiber: 1.6, minerals: {} } },
  { name: 'борщ с мясом', aliases: ['борщ с мясом', 'борщ с говядиной', 'борщ со свининой'], source: 'skurikhin', prepared: true,
    per100: { kcal: 63, prot: 4.4, fat: 3.6, carb: 5.5, fiber: 1.3, minerals: {} } },
  { name: 'щи', aliases: ['щи', 'щи из свежей капусты'], source: 'skurikhin', prepared: true,
    per100: { kcal: 38, prot: 1, fat: 3.8, carb: 2.1, fiber: 0.8, minerals: {} } },
  { name: 'щи из квашеной капусты', aliases: ['щи из квашеной капусты', 'щи кислые'], source: 'skurikhin', prepared: true,
    per100: { kcal: 32, prot: 0.6, fat: 2.1, carb: 2.7, fiber: 1.4, minerals: {} } },
  { name: 'солянка', aliases: ['солянка', 'солянка мясная', 'солянка сборная'], source: 'skurikhin', prepared: true,
    per100: { kcal: 69, prot: 5.2, fat: 4.6, carb: 1.7, fiber: 0.4, minerals: {} } },
  { name: 'рассольник', aliases: ['рассольник'], source: 'skurikhin', prepared: true,
    per100: { kcal: 42, prot: 1.4, fat: 2, carb: 5, fiber: 0.8, minerals: {} } },
  { name: 'окрошка', aliases: ['окрошка', 'окрошка на квасе'], source: 'skurikhin', prepared: true,
    per100: { kcal: 52, prot: 2.1, fat: 1.7, carb: 6.3, fiber: 0.5, minerals: {} } },
  { name: 'окрошка на кефире', aliases: ['окрошка на кефире'], source: 'skurikhin', prepared: true,
    per100: { kcal: 47, prot: 3.1, fat: 1.9, carb: 4.3, fiber: 0.5, minerals: {} } },
  { name: 'уха', aliases: ['уха', 'рыбный суп'], source: 'skurikhin', prepared: true,
    per100: { kcal: 46, prot: 3.4, fat: 1, carb: 5.5, fiber: 0.5, minerals: {} } },
  { name: 'гороховый суп', aliases: ['гороховый суп', 'суп гороховый'], source: 'skurikhin', prepared: true,
    per100: { kcal: 66, prot: 4.4, fat: 2.4, carb: 8.9, fiber: 2, minerals: {} } },
  { name: 'грибной суп', aliases: ['грибной суп', 'суп грибной'], source: 'skurikhin', prepared: true,
    per100: { kcal: 50, prot: 1.9, fat: 2.4, carb: 5.7, fiber: 1.1, minerals: {} } },
  { name: 'суп харчо', aliases: ['харчо', 'суп харчо'], source: 'skurikhin', prepared: true,
    per100: { kcal: 75, prot: 3.1, fat: 4.5, carb: 5.5, fiber: 0.8, minerals: {} } },
  { name: 'свекольник', aliases: ['свекольник', 'холодник'], source: 'skurikhin', prepared: true,
    per100: { kcal: 36, prot: 0.5, fat: 2, carb: 4.2, fiber: 0.5, minerals: {} } },
  { name: 'щавелевый суп', aliases: ['щавелевый суп', 'суп из щавеля'], source: 'skurikhin', prepared: true,
    per100: { kcal: 40, prot: 1.6, fat: 2.5, carb: 2.9, fiber: 0.5, minerals: {} } },
  { name: 'фасолевый суп', aliases: ['фасолевый суп', 'суп фасолевый'], source: 'skurikhin', prepared: true,
    per100: { kcal: 62, prot: 4, fat: 1.8, carb: 10, fiber: 2.6, minerals: {} } },
  { name: 'куриный бульон', aliases: ['бульон', 'куриный бульон'], source: 'skurikhin', prepared: true,
    per100: { kcal: 15, prot: 2, fat: 0.5, carb: 0.3, fiber: 0, minerals: {} } },

  // — Салаты —
  { name: 'оливье', aliases: ['оливье', 'салат оливье', 'зимний салат'], source: 'skurikhin', prepared: true,
    per100: { kcal: 115, prot: 4.6, fat: 8, carb: 5.9, fiber: 1.3, minerals: {} } },
  { name: 'винегрет', aliases: ['винегрет'], source: 'skurikhin', prepared: true,
    per100: { kcal: 75, prot: 1.8, fat: 3.7, carb: 8.8, fiber: 2.2, minerals: {} } },
  { name: 'селёдка под шубой', aliases: ['селёдка под шубой', 'сельдь под шубой', 'шуба'], source: 'skurikhin', prepared: true,
    per100: { kcal: 193, prot: 5.1, fat: 16.2, carb: 7.3, fiber: 1.6, minerals: {} } },
  { name: 'салат мимоза', aliases: ['мимоза', 'салат мимоза'], source: 'skurikhin', prepared: true,
    per100: { kcal: 183, prot: 5.7, fat: 14.8, carb: 7.2, fiber: 1.1, minerals: {} } },
  { name: 'крабовый салат', aliases: ['крабовый салат', 'салат с крабовыми палочками'], source: 'skurikhin', prepared: true,
    per100: { kcal: 128, prot: 9.2, fat: 7.4, carb: 5.9, fiber: 0.5, minerals: {} } },

  // — Вторые блюда —
  { name: 'котлета', aliases: ['котлета', 'котлеты', 'котлета мясная', 'котлета говяжья'], source: 'skurikhin', prepared: true,
    per100: { kcal: 260, prot: 18, fat: 20, carb: 0, fiber: 0.1, minerals: {} } },
  { name: 'котлета из индейки', aliases: ['котлета из индейки', 'котлеты индюшиные'], source: 'skurikhin', prepared: true,
    per100: { kcal: 220, prot: 18.6, fat: 12.2, carb: 8.7, fiber: 0.7, minerals: {} } },
  { name: 'гуляш', aliases: ['гуляш', 'гуляш говяжий'], source: 'skurikhin', prepared: true,
    per100: { kcal: 148, prot: 14, fat: 9.2, carb: 2.6, fiber: 0.3, minerals: {} } },
  { name: 'голубцы', aliases: ['голубцы', 'голубец'], source: 'skurikhin', prepared: true,
    per100: { kcal: 130, prot: 5, fat: 9, carb: 6, fiber: 1.7, minerals: {} } },
  { name: 'бефстроганов', aliases: ['бефстроганов'], source: 'skurikhin', prepared: true,
    per100: { kcal: 193, prot: 16.7, fat: 11.3, carb: 5.9, fiber: 0.9, minerals: {} } },
  { name: 'азу', aliases: ['азу'], source: 'skurikhin', prepared: true,
    per100: { kcal: 214, prot: 11.9, fat: 14.2, carb: 10.2, fiber: 1.7, minerals: {} } },
  { name: 'сырники', aliases: ['сырники', 'сырник', 'творожники'], source: 'skurikhin', prepared: true,
    per100: { kcal: 183, prot: 18.6, fat: 3.6, carb: 18.2, fiber: 0.8, minerals: {} } },
  // Гороховая каша (2026-08-25: класса не было — фото-разбор уезжал в USDA и
  // возвращал «чечевицу варёную»). Пюре из лущёного гороха НА ВОДЕ без масла =
  // варёный горох (USDA split peas cooked #16086: Б 8.3, Ж 0.4, У 21.1 общий,
  // клетчатка 8.3); ккал по единой формуле, не измеренные USDA 118.
  { name: 'гороховая каша', aliases: ['гороховая каша', 'каша гороховая', 'гороховое пюре', 'пюре гороховое', 'гороховая каша на воде'], source: 'skurikhin', prepared: true,
    per100: { kcal: 105, prot: 8.3, fat: 0.4, carb: 21.1, fiber: 8.3, minerals: {} } },

  // — Каши на молоке —
  { name: 'гречневая каша на молоке', aliases: ['гречневая каша на молоке', 'гречка с молоком'], source: 'skurikhin', prepared: true,
    per100: { kcal: 118, prot: 4.2, fat: 2.3, carb: 21.6, fiber: 3, minerals: {} } },
  { name: 'манная каша', aliases: ['манная каша', 'манка', 'манная каша на молоке'], source: 'skurikhin', prepared: true,
    per100: { kcal: 98, prot: 3, fat: 3.2, carb: 15.3, fiber: 0.6, minerals: {} } },
  { name: 'овсяная каша на молоке', aliases: ['овсяная каша на молоке', 'овсянка на молоке'], source: 'skurikhin', prepared: true,
    per100: { kcal: 102, prot: 3.2, fat: 4.1, carb: 14.2, fiber: 1.7, minerals: {} } },

  // — Напитки —
  { name: 'компот из сухофруктов', aliases: ['компот', 'компот из сухофруктов'], source: 'skurikhin', prepared: true,
    per100: { kcal: 60, prot: 0.8, fat: 0, carb: 14.2, fiber: 0.5, minerals: {} } },
  { name: 'кисель', aliases: ['кисель'], source: 'skurikhin', prepared: true,
    per100: { kcal: 78, prot: 0.2, fat: 0, carb: 18.9, fiber: 0.5, minerals: {} } },
  // Сладкая газировка. Класс отсутствовал целиком, и на «лимонад тархун
  // черноголовка» цепочка уходила в USDA, где «тархун» матчился в СУШЁНЫЙ
  // ЭСТРАГОН (295 ккал, 22.8 г белка) — бутылка лимонада выходила на 974 ккал.
  // Названия сортов («тархун», «дюшес», «байкал», «саяны») — это травы и ягоды,
  // поэтому именно они и ловят травяные строки из англоязычных баз; держим их
  // здесь, первыми в цепочке. Бренд в запросе («черноголовка») не мешает: строка
  // берётся по названию сорта. Значения — типовая сладкая газировка; конкретный
  // продукт пользователь поправит через «Другой вариант».
  { name: 'лимонад', aliases: ['лимонад', 'газировка', 'ситро'], source: 'skurikhin', prepared: true,
    per100: { kcal: 38, prot: 0, fat: 0, carb: 9.5, fiber: 0, minerals: {} } },
  { name: 'лимонад тархун', aliases: ['тархун', 'лимонад тархун', 'напиток тархун'], source: 'skurikhin', prepared: true,
    per100: { kcal: 35, prot: 0, fat: 0, carb: 8.7, fiber: 0, minerals: {} } },
  { name: 'лимонад дюшес', aliases: ['дюшес', 'лимонад дюшес'], source: 'skurikhin', prepared: true,
    per100: { kcal: 38, prot: 0, fat: 0, carb: 9.4, fiber: 0, minerals: {} } },
  { name: 'лимонад байкал', aliases: ['байкал', 'лимонад байкал'], source: 'skurikhin', prepared: true,
    per100: { kcal: 36, prot: 0, fat: 0, carb: 9, fiber: 0, minerals: {} } },
  { name: 'лимонад саяны', aliases: ['саяны', 'лимонад саяны'], source: 'skurikhin', prepared: true,
    per100: { kcal: 37, prot: 0, fat: 0, carb: 9.2, fiber: 0, minerals: {} } },
  { name: 'квас', aliases: ['квас', 'квас хлебный'], source: 'skurikhin', prepared: true,
    per100: { kcal: 27, prot: 0.2, fat: 0, carb: 5.2, fiber: 0.1, minerals: {} } },
  { name: 'морс', aliases: ['морс', 'морс клюквенный', 'морс брусничный'], source: 'skurikhin', prepared: true,
    per100: { kcal: 41, prot: 0.1, fat: 0, carb: 10.1, fiber: 0.2, minerals: {} } },

  // — Выпечка / сладкое —
  { name: 'пончик', aliases: ['пончик', 'пончики', 'пышка', 'пышки'], source: 'skurikhin', prepared: true,
    per100: { kcal: 296, prot: 5.8, fat: 13, carb: 38.8, fiber: 0.6, minerals: {} } },
  { name: 'булочка', aliases: ['булочка', 'булка', 'сдоба', 'сдобная булочка'], source: 'skurikhin', prepared: true,
    per100: { kcal: 339, prot: 7.9, fat: 9.4, carb: 55.5, fiber: 3.4, minerals: {} } },
  { name: 'пирожок', aliases: ['пирожок', 'пирожки'], source: 'skurikhin', prepared: true,
    per100: { kcal: 294, prot: 7.7, fat: 7, carb: 50, fiber: 2.3, minerals: {} } },
  { name: 'блины', aliases: ['блины', 'блин', 'блинчик', 'блинчики'], source: 'skurikhin', prepared: true,
    per100: { kcal: 233, prot: 6.1, fat: 12.3, carb: 26, fiber: 1, minerals: {} } },
  { name: 'оладьи', aliases: ['оладьи', 'оладья', 'оладушки'], source: 'skurikhin', prepared: true,
    per100: { kcal: 295, prot: 6.4, fat: 9, carb: 46, fiber: 1.6, minerals: {} } },
  // Пельмени — deliberately NOT `prepared`: people boil or fry them at home,
  // so the cooking-method chips stay genuinely useful here.
  { name: 'пельмени', aliases: ['пельмени', 'пельмень'], source: 'skurikhin',
    per100: { kcal: 275, prot: 11.9, fat: 12.4, carb: 29, fiber: 1.9, minerals: {} } },
  { name: 'печенье', aliases: ['печенье', 'печеньки'], source: 'skurikhin', prepared: true,
    per100: { kcal: 417, prot: 7.5, fat: 11.8, carb: 74.4, fiber: 2, minerals: {} } },

  // — Уличная еда и быстрые блюда (UX-аудит 2026-08-20: топ повседневных
  //   запросов, дававших ПУСТО в RU-таблицах — уходили в EN-цепочку/ИИ). Те же
  //   editorial reference figures, что и выше; вариативность рецептов лечит
  //   «не то?»-пикер. —
  { name: 'плов', aliases: ['плов', 'плов с мясом', 'плов с говядиной', 'плов с бараниной'], source: 'skurikhin', prepared: true,
    per100: { kcal: 165, prot: 8.4, fat: 6.4, carb: 18.6, fiber: 1, minerals: {} } },
  { name: 'плов с курицей', aliases: ['плов с курицей', 'куриный плов'], source: 'skurikhin', prepared: true,
    per100: { kcal: 142, prot: 9.5, fat: 4.4, carb: 16, fiber: 1, minerals: {} } },
  { name: 'шаурма', aliases: ['шаурма', 'шаверма', 'шаурма с курицей', 'донер', 'донер кебаб'], source: 'skurikhin', prepared: true,
    per100: { kcal: 198, prot: 9, fat: 10, carb: 18, fiber: 1.4, minerals: {} } },
  { name: 'лапша быстрого приготовления', aliases: ['лапша быстрого приготовления', 'доширак', 'ролтон', 'лапша с приправами'], source: 'skurikhin', prepared: true,
    per100: { kcal: 134, prot: 3, fat: 6, carb: 17, fiber: 0.9, minerals: {} } },
  { name: 'сосиска в тесте', aliases: ['сосиска в тесте'], source: 'skurikhin', prepared: true,
    per100: { kcal: 289, prot: 9, fat: 17, carb: 25, fiber: 1.2, minerals: {} } },
  { name: 'беляш', aliases: ['беляш', 'беляши', 'беляш с мясом'], source: 'skurikhin', prepared: true,
    per100: { kcal: 320, prot: 10, fat: 20, carb: 25, fiber: 1.1, minerals: {} } },
  { name: 'чебурек', aliases: ['чебурек', 'чебуреки', 'чебурек с мясом'], source: 'skurikhin', prepared: true,
    per100: { kcal: 261, prot: 9, fat: 13, carb: 27, fiber: 1.2, minerals: {} } },
  { name: 'хинкали', aliases: ['хинкали'], source: 'skurikhin', prepared: true,
    per100: { kcal: 228, prot: 10, fat: 12, carb: 20, fiber: 1.1, minerals: {} } },
  { name: 'хачапури', aliases: ['хачапури', 'хачапури с сыром', 'хачапури по имеретински'], source: 'skurikhin', prepared: true,
    per100: { kcal: 270, prot: 9.5, fat: 12, carb: 31, fiber: 1.2, minerals: {} } },
  { name: 'бутерброд с колбасой', aliases: ['бутерброд с колбасой', 'бутерброд с копченой колбасой'], source: 'skurikhin', prepared: true,
    per100: { kcal: 267, prot: 9, fat: 15, carb: 24, fiber: 1.3, minerals: {} } },
  { name: 'бутерброд с сыром', aliases: ['бутерброд с сыром', 'бутерброд с маслом и сыром'], source: 'skurikhin', prepared: true,
    per100: { kcal: 270, prot: 11, fat: 14, carb: 25, fiber: 1.2, minerals: {} } },

  // — Отруби (2026-08-22: класса не было ни в одной RU-таблице). Сухой ПРОДУКТ,
  //   не блюдо: prepared не ставим. БЖУ — USDA SR (oat bran raw #08029, wheat
  //   bran crude #20077), carb общий (с волокнами).
  //
  //   ККАЛ — ПО ЕДИНОЙ ФОРМУЛЕ (ТР ТС 022/2011), а не измеренные USDA 246/216.
  //   USDA считает отруби по специфическим факторам Этуотера (плохая усвояемость
  //   волокон), и её число на треть ниже того, что напечатано на российской
  //   пачке. Человек сверяет карточку с пачкой в руке, а вся таблица построена
  //   на одной формуле (docs/nutrition-science.md §1) — разнобой внутри таблицы
  //   хуже, чем расхождение с USDA. Проверяется тестом energy-консистентности. —
  { name: 'отруби овсяные', aliases: ['отруби овсяные', 'овсяные отруби'], source: 'skurikhin',
    per100: { kcal: 366, prot: 17.3, fat: 7, carb: 66.2, fiber: 15.4, minerals: {} } },
  { name: 'отруби пшеничные', aliases: ['отруби пшеничные', 'пшеничные отруби', 'отруби'], source: 'skurikhin',
    per100: { kcal: 274, prot: 15.6, fat: 4.3, carb: 64.5, fiber: 42.8, minerals: {} } },

  // — Творожные сырки (2026-09-03: класса не было ни в одной RU-таблице).
  //   «глазированный сырок» уходил в FatSecret по name_en «glazed cottage
  //   cheese bar» и садился на «Творог» 87 ккал/100 г — сырок в шоколаде
  //   становился творогом, вчетверо легче настоящего (скрин владельца: 39 ккал
  //   за штуку вместо ~180). Скурихин: «сырки глазированные» (27,8 г жира)
  //   и «сырки и масса творожные особые» (сладкие, без глазури). Готовый
  //   продукт, едят как есть — prepared. Голый «сырок» осознанно НЕ алиас:
  //   слово двусмысленно (плавленый сырок — сыр, а не творог); по частичному
  //   совпадению обе строки всё равно поднимаются, а выбор — за человеком. —
  { name: 'сырок глазированный', aliases: ['сырок глазированный', 'глазированный сырок', 'сырок в глазури', 'сырок в шоколаде', 'сырок творожный глазированный', 'творожный сырок глазированный', 'глазированный творожный сырок', 'сырки глазированные', 'глазированные сырки'], source: 'skurikhin', prepared: true,
    per100: { kcal: 407, prot: 8.5, fat: 27.8, carb: 32, fiber: 0, minerals: {} } },
  { name: 'сырок творожный', aliases: ['сырок творожный', 'творожный сырок', 'сырок творожный сладкий', 'сырок без глазури', 'сырки творожные', 'творожная масса', 'масса творожная', 'сырковая масса'], source: 'skurikhin', prepared: true,
    per100: { kcal: 341, prot: 7.1, fat: 23, carb: 27.5, fiber: 0, minerals: {} } },
];
